...
import sift_pyx12.error_997
import sift_pyx12.error_999
import sift_pyx12.error_html
import sift_pyx12.errors
import sift_pyx12.map_index
import sift_pyx12.map_if
import sift_pyx12.x12file
from sift_pyx12.map_walker import walk_tree
import sift_pyx12.x12xml_simple
...
def x12n_document(param, src_file, fd_997, fd_html,
    ...
    @param param: sift_pyx12.param instance
    ...
    errh = sift_pyx12.error_handler.err_handler()

    # Get X12 DATA file
    try:
        src = sift_pyx12.x12file.X12Reader(src_file)
    # pam.caton adding more detailed messaging here. 9/14/2021
    except sift_pyx12.errors.X12Error as e:
        print(str(e))
    ...
    control_map = sift_pyx12.map_if.load_map_file(map_file, param, map_path)
    map_index_if = sift_pyx12.map_index.map_index(map_path)
    ...
    if fd_html:
        html = sift_pyx12.error_html.error_html(errh, fd_html, src.get_term())
        html.header()
        err_iter = sift_pyx12.error_handler.err_iter(errh)
    if fd_xmldoc:
        xmldoc = sift_pyx12.x12xml_simple.x12xml_simple(fd_xmldoc, param.get('simple_dtd'))
    ...
    for seg in src:
        ...
        if seg.get_seg_id() == 'ISA':
            ...
            except sift_pyx12.errors.EngineError:
            ...
            if seg.get_seg_id() == 'ISA':
                ...
                if map_file != map_file_new:
                    ...
                    if map_file is None:
                        ...
                        raise sift_pyx12.errors.EngineError(err_str)
                    cur_map = sift_pyx12.map_if.load_map_file(map_file, param, map_path)
        ...
        if fd_html:
            ...
            while True:
                ...
                except sift_pyx12.errors.IterOutOfBounds:
                    break
    ...
    #visit_debug = sift_pyx12.error_debug.error_debug_visitor(sys.stdout)
    ...
    if fd_997 and fic != 'FA':
        if vriic and vriic[:6] == '004010':
            try:
                visit_997 = sift_pyx12.error_997.error_997_visitor(fd_997, src.get_term())
        ...
        if vriic and vriic[:6] == '005010':
            try:
                visit_999 = sift_pyx12.error_999.error_999_visitor(fd_997, src.get_term())
