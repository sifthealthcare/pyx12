...
class X12LoopDataNode(X12DataNode):
    ...
    def add_segment(self, seg_data):
        ...
        @raise sift_pyx12.errors.X12PathError: If invalid segment
        @todo: Check counts?
        """
    ...
    def _get_segment(self, seg_obj):
        """
        Get a sift_pyx12.segment.Segment instance, building one from a string
        """
        if isinstance(seg_obj, sift_pyx12.segment.Segment):
            return seg_obj
            ...
            return sift_pyx12.segment.Segment(seg_obj, seg_term, ele_term, subele_term)
        else:
            raise errors.EngineError('Unknown type %s for seg_obj %i.  Expecting a sift_pyx12.segment.Segment or a str'
                ...
                if seg_id == 'ISA':
                    ...
                    if self.map_file != map_file_new:
                        ...
                        if self.map_file is None:
                            raise sift_pyx12.errors.EngineError("Map not found.  icvn=%s, fic=%s, vriic=%s" %
                                                           (icvn, fic, vriic))
