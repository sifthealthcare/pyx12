...
class err_handler(object):
    ...
    def __init__(self, xml_out=None, basedir=None):
        ...
        """
        self.logger = logging.getLogger('sift_pyx12.errh_xml')
        if xml_out:
            self.filename = xml_out
            self.fd = open(xml_out, 'w')
        else:
            try:
                (fdesc, self.filename) = tempfile.mkstemp('.xml', 'sift_pyx12_')
                self.fd = os.fdopen(fdesc, 'w+b')
            except:
                (fdesc, self.filename) = tempfile.mkstemp(suffix='.xml', prefix='sift_pyx12_', dir=basedir)
                self.fd = os.fdopen(fdesc, 'w+b')
        self.cur_line = None
        self.errors = []
        if not self.fd:
