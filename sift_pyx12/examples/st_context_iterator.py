...
def simple_reader():
    testfile = 'multiple_st_loops.txt'
    src = sift_pyx12.x12file.X12Reader(testfile)
...
def x12_split_on_st(source_filename, isa_id=11, gs_id=21):
    src = sift_pyx12.x12file.X12Reader(source_filename)
    idx = -1
    for k, g in groupby(get_headers_stream(src), lambda x: x[0]):
        idx += 1
        st_id = int(k['st_seg'].get_value('ST02'))
        fd_temp = tempfile.TemporaryFile(mode='w+', encoding='ascii')
        wr = sift_pyx12.x12file.X12Writer(fd_temp, '~', '*', ':', '\n', '^')
...
def iterate_2000(fd_in):
    ...
    src = sift_pyx12.x12context.X12ContextReader(param, errh, fd_in)
    #isa_id = None
    #gs_id = None
    isa_seg = None
    gs_seg = None
