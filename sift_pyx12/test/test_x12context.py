...
from sift_pyx12.errors import EngineError  # , X12PathError
import sift_pyx12.x12context
import sift_pyx12.params
from sift_pyx12.test.x12testdata import datafiles
...
class Delimiters(X12fileTestCase):
    ...
    def test_arbitrary_delimiters(self):
        ...
        src = sift_pyx12.x12context.X12ContextReader(self.param, errh, fd)
...
class TreeGetValue(X12fileTestCase):
    ...
    def setUp(self):
        ...
        errh = sift_pyx12.error_handler.errh_null()
        self.src = sift_pyx12.x12context.X12ContextReader(param, errh, fd)
...
class TreeSelectFromSegment(X12fileTestCase):
    ...
    def test_select_from_seg_fail(self):
        ...
        param = sift_pyx12.params.params()
        errh = sift_pyx12.error_handler.errh_null()
        src = sift_pyx12.x12context.X12ContextReader(param, errh, fd)
        for datatree in src.iter_segments('ST_LOOP'):
            if datatree.id == 'GS':
...
class TreeAddSegment(X12fileTestCase):
    ...
    def test_add_new_plain(self):
        seg_data = sift_pyx12.segment.Segment('HCP*00*7.11~', '~', '*', ':')
        new_node = self.loop2300.add_segment(seg_data)
        self.assertNotEqual(new_node, None)

    def test_add_new_id(self):
        seg_data = sift_pyx12.segment.Segment('REF*F5*6.11~', '~', '*', ':')
        new_node = self.loop2300.add_segment(seg_data)
        self.assertNotEqual(new_node, None)

    def test_add_new_not_exists(self):
        seg_data = sift_pyx12.segment.Segment('ZZZ*00~', '~', '*', ':')
        self.assertRaises(sift_pyx12.errors.X12PathError,
...
class SegmentExists(X12fileTestCase):
    ...
    def setUp(self):
        ...
        self.param = sift_pyx12.params.params()
        errh = sift_pyx12.error_handler.errh_null()
        self.src = sift_pyx12.x12context.X12ContextReader(self.param, errh, fd)
        for datatree in self.src.iter_segments('2300'):
            if datatree.id == '2300':
                self.loop2300 = datatree
                break
...
class TreeAddNode(X12fileTestCase):
    ...
    def test_fail(self):
        ...
        self.assertRaises(sift_pyx12.errors.X12PathError, n2400.add_node, cn1)
...
class TreeDeleteSegment(X12fileTestCase):
    ...
    def test_delete(self):
        ...
        seg_data = sift_pyx12.segment.Segment('CN1*05~', '~', '*', ':')
