...
class IsValidSyntax(unittest.TestCase):
    ...
    def test_fail_bad_syntax(self):
        seg1 = ['MEA', 'OG', 'HT', '', '', '', '', '', '']
        seg = sift_pyx12.segment.Segment('*'.join(seg1), '~', '*', ':')
        syntax = ['R', 3]
        (result, err_str) = sift_pyx12.map_if.is_syntax_valid(seg, syntax)
...
class IsValidSyntaxL(unittest.TestCase):
    ...
    def test_L_missing_element_ok(self):
        seg = sift_pyx12.segment.Segment(
