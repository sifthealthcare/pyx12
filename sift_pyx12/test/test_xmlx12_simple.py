...
import sift_pyx12.xmlx12_simple
...
class XmlTransformTestCase(unittest.TestCase):
    ...
    def _isX12Diff(self, fd1, fd2):
        ...
        """
        src1 = sift_pyx12.x12file.X12Reader(fd1)
        src2 = sift_pyx12.x12file.X12Reader(fd2)
    ...
    def _test_x12xml_simple(self, datakey):
        ...
        (fdesc, filename) = tempfile.mkstemp('.xml', 'sift_pyx12_')
        with open(filename, 'r+') as fd_xml:
            # fd_xml = tempfile.TemporaryFile(mode='w+', encoding='ascii')
            fd_result = StringIO()
            self.param.set('xmlout', 'simple')
            result = sift_pyx12.x12n_document.x12n_document(param=self.param, src_file=fd_source,
                fd_997=None, fd_html=None, fd_xmldoc=fd_xml, xslt_files=None)
            self.assertTrue(result)
            fd_xml.seek(0)
            fd_result.seek(0)
            result = sift_pyx12.xmlx12_simple.convert(fd_xml, fd_result)
            fd_source.seek(0)
            fd_result.seek(0)
