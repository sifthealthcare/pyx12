...
def main():
    ...
    for file_in in args.input_files:
        ...
        # pam.caton 9/13/2021 updated per issue 68: https://github.com/azoner/pyx12/issues/68
        fd_out = tempfile.TemporaryFile(mode='r+')
        ...
        fd_out.seek(0)
        if args.outputfile:
            # pam.caton changed to actually write to file
            # fd_out = codecs.open(args.outputfile, mode='w', encoding='ascii')
            with codecs.open(args.outputfile, mode='w', encoding='ascii') as fd_new:
                fd_new.write(fd_out.read())
        else:
            if args.inplace:
